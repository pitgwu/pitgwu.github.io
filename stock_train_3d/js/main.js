...
(function (global) {
  ...
  // 引入三日模組
  const Strat3 = global.Strategy3Day;

  // 全域變數
  let threeDayVisible = false;
  ...
  function updateDisplays() {
    ...
    let stratData = null;
    if (threeDayVisible) {
      stratData = Strat3.calculate(shown);
    }
  ...
  function bindEvents() {
    ...
    // 綁定三日按鈕事件
    U.el("toggle3Day").onclick = () => {
      threeDayVisible = !threeDayVisible;
      const btn = U.el("toggle3Day");
      if (threeDayVisible) {
        btn.innerText = "三日高低：ON";
        btn.style.background = "#fffacd"; // 亮黃色
        btn.style.borderColor = "#ffd700";
      } else {
        btn.innerText = "三日高低：OFF";
